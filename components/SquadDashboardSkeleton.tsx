    ...
    <div className="space-y-4 sm:space-y-8">
      {Array.from({ length: memberBlocks }).map((_, mi) => (
        <div key={mi} className="space-y-1.5 sm:space-y-2">
          ...
          <div className="overflow-x-auto rounded-lg border border-zinc-200/80 bg-zinc-50/30 sm:rounded-xl dark:border-white/10 dark:bg-[#262626]">
            <table className="min-w-full border-collapse text-xs sm:text-sm">
              <thead>
                <tr className="bg-emerald-50/80 dark:bg-[#262626]">
                  <th className="border border-zinc-200/80 px-1.5 py-1.5 text-left align-middle w-28 sm:w-32 sm:px-2 sm:py-2 dark:border-white/10">
                    <Skeleton className="h-3 w-12 sm:h-3.5 sm:w-14" />
                  </th>
                  {Array.from({ length: dayCells }).map((_, di) => (
                    <th
                      key={di}
                      className="border border-zinc-200/80 px-0.5 py-1.5 align-middle sm:px-1 sm:py-2 dark:border-white/10"
                    >
              ...
              <tbody>
                {Array.from({ length: habitRows }).map((_, ri) => (
                  <tr key={ri}>
                    <td className="border border-zinc-200/80 px-1.5 py-1.5 align-middle bg-white sm:px-2 sm:py-2 dark:border-white/10 dark:bg-[#262626]">
                      <Skeleton className="h-3 w-24 sm:h-3.5 sm:w-28" />
                    </td>
                    {Array.from({ length: dayCells }).map((_, di) => (
                      <td
                        key={di}
                        className="border border-zinc-200/80 px-0.5 py-1.5 text-center align-middle bg-white sm:px-1 sm:py-2 dark:border-white/10 dark:bg-[#262626]"
                      >
